...
def create_fastwam_3dmask(
    ...
    from .models.wan22.fastwam_3dmask import FastWAM3DMask
    ...
    if isinstance(visibility, DictConfig):
        visibility = OmegaConf.to_container(visibility, resolve=True)
    if visibility is None:
        visibility = {}
    if not isinstance(visibility, dict):
        raise ValueError(f"`visibility` must be dict-like, got {type(visibility)}")

    return FastWAM3DMask.from_wan22_pretrained(
        device=device,
        torch_dtype=model_dtype,
        model_id=model_id,
        tokenizer_model_id=tokenizer_model_id,
